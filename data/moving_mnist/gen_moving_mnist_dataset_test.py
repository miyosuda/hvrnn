...
class GenMovingMNISTDatasetTest(unittest.TestCase):
  def test_generate(self):
    data_path = "./moving_mnist1.npz"
    data_all = np.load(data_path)

    data_images    = data_all["images"]    # (14000, 20, 64, 64) uint8
    data_labels    = data_all["labels"]    # (14000, 1)          uint8
    data_pos_xs    = data_all["pos_xs"]    # (14000, 20, 1)      float32
    data_pos_ys    = data_all["pos_ys"]    # (14000, 20, 1)      float32
    data_speeds    = data_all["speeds"]    # (14000, 1)          unit8
    ...
    self.assertEqual(data_labels.shape,    (14000,1))
    self.assertEqual(data_pos_xs.shape,    (14000,20,1))
    self.assertEqual(data_pos_ys.shape,    (14000,20,1))
    self.assertEqual(data_speeds.shape,    (14000,1))
    self.assertEqual(data_bounce_xs.shape, (14000,20,1))
    self.assertEqual(data_bounce_ys.shape, (14000,20,1))
